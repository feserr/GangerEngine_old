...
public:
    Gun(std::string name, int fireRate, int bulletsPerShot, float spread, float bulletDamage,
        float bulletSpeed, GangerEngine::SoundEffect effect);
    ~Gun();

    void update(bool isMouseDown, const glm::vec2& position, const glm::vec2& direction, std::vector<Bullet>& bullets, float timeDelta);
...
private:
    ...
    GangerEngine::SoundEffect m_effect;
};
