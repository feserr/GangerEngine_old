...
void Player::Draw(GangerEngine::SpriteBatch& spriteBatch) {
    ...
    spriteBatch.Draw(destRect, uvRectBottom, m_texture.texture.id, 0.0f, m_color);
...
void Player::DrawDebug(GangerEngine::DebugRenderer& debugRenderer) {
    GangerEngine::ColorRGBA8 color(255, 255, 0, 255);
    // Draw box
    glm::vec4 destRect;
    destRect.x = m_position.x - m_drawDims.x / 2.0f;
    destRect.y = m_position.y - m_drawDims.y / 2.0f;
...
void Player::Update(float deltaTime, GangerEngine::InputManager& inputManager,
